...
I will use a Phyloseq-Object to analyse the 18S rDNA sequening data. For this script we will need the seqtab.nochim-table, generated in 02a_DADA2_18S_concatination.Rmd, the taxa-table from the 02b_Annotation_18S_PR2.Rmd script and the metadata, which we will upload within this script. 
...
```{r}
taxa <- readRDS("/home/sukmb626/taxa_all.rds" # where you stored your taxonomic annotation to PR2
taxonomy <- taxa 
```
...
samples_cleaned <- samples[!is.na(samples$id_ngs18s), ] #remove samples, that don't have a id_ngs_18S
...
##if this is not TRUE, remove unidenntical rownames
